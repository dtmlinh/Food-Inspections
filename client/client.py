...
def run_loader(server, port, insp_file, tweet_file, index_timing, load_type, halt_on_error=False, limit=None, clean=False):
    ...
    if insp_file:
        ...
        logging.info("Skipping Inspections")
        insp_time = 0
    ...
    if tweet_file:
        ...
        tweet_time = tweet_hist.get_mean_value() * tweet_hist.get_total_count()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--inspfile", dest="insp_file", help="Input JSON/CSV of inspections")
    ...
    if config.insp_file and not (config.insp_file.endswith('.json') and isinstance(config.load_type, int)) and\
